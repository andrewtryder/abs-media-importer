...
`reeldock` can be deployed in a Proxmox VE environment as either a dedicated Virtual Machine (VM) running Debian or inside a Linux Container (LXC).
...
SSH into the guest VM, open `/opt/reeldock/.env`, and configure:
```env
...
In a native systemd deployment, open `/etc/reeldock/.env` and update the output root directly:
```env
